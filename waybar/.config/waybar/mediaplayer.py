...
class PlayerManager:
    def __init__(self, selected_player=None, excluded_player=[]):
        ...
        GLib.timeout_add(500, self.truncate_title)
    ...
    def truncate_title(self):
        ...
        if track_info:
            if self.curr_player.props.status == "Playing":
                track_info = " " + track_info
            else:
                track_info = " " + track_info
        # only print output if no other player is playing
        current_playing = self.get_first_playing_player()
        if current_playing is None or current_playing.props.player_name == self.curr_player.props.player_name:
            self.write_output(track_info, self.curr_player)
        else:
    ...
    def build_scrolling_text(text: str) -> str:
        gap = "     "  # space between loops
        repeated = (text + gap) * 2  # Repeat it to simulate continuous scroll
        return f"<span class='scrolling-text'>{repeated}</span>"
    ...
    def on_metadata_changed(self, player, metadata, _=None):
        ...
        self.curr_track_title = track_info
